...
export async function POST(
  request: Request,
  context: { params: Promise<{ id: string; empenhoId: string }> },
) {
  ...
  if (typeof category !== 'string' || !category) {
    return NextResponse.json({ message: 'Categoria do arquivo nao informada.' }, { status: 400 });
  }

  const timestamp = Date.now();
  const safeFileName = sanitizeFileName(file.name);
  const relativeDirectory = path.join('uploads', 'licitacoes', id, 'empenhos', empenhoId);
  ...
  const response = await fetch(`${API_URL}/api/licitacoes/${id}/empenhos/${empenhoId}/anexos`, {
    method: 'POST',
    headers: {
  ...
  const data = await response.json().catch(() => ({ message: 'Nao foi possivel registrar o anexo do empenho.' }));
