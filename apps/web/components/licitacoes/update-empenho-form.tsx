...
export function UpdateEmpenhoForm({ licitacaoId, empenho, onSuccess, onCancel }: UpdateEmpenhoFormProps) {
  const router = useRouter();
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();
  const [codigoEmpenho, setCodigoEmpenho] = useState(empenho.codigoEmpenho);
  const [valor, setValor] = useState(formatCurrencyEditValue(empenho.valor));
  const [isValorFocused, setIsValorFocused] = useState(false);
  const [dataEmpenho, setDataEmpenho] = useState(empenho.dataEmpenho?.slice(0, 10) ?? '');
  const [dataPagamentoEmpenho, setDataPagamentoEmpenho] = useState(empenho.dataPagamentoEmpenho?.slice(0, 10) ?? '');
  const [dataGeracaoBoleto, setDataGeracaoBoleto] = useState(empenho.dataGeracaoBoleto?.slice(0, 10) ?? '');
  const [dataPagamentoBoleto, setDataPagamentoBoleto] = useState(empenho.dataPagamentoBoleto?.slice(0, 10) ?? '');
  const [observacoes, setObservacoes] = useState(empenho.observacoes ?? '');

  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setError(null);

    const response = await fetch(`/api/licitacoes/${licitacaoId}/empenhos/${empenho.id}`, {
      method: 'PATCH',
      headers: {
    ...
    if (!response.ok) {
      const data = await response.json().catch(() => ({ message: 'Nao foi possivel atualizar o empenho.' }));
      setError(data.message ?? 'Nao foi possivel atualizar o empenho.');
      return;
    }
  ...
  return (
    ...
    >
      <div>
        <h2 style={{ margin: '0 0 6px' }}>Editar empenho</h2>
        <p style={{ margin: 0, color: 'var(--muted)' }}>Atualize os dados do empenho vinculado a esta licitacao.</p>
      </div>

      <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
        <label style={fieldStyle}>
          <span>Empenho</span>
          <input value={codigoEmpenho} onChange={(event) => setCodigoEmpenho(event.target.value)} required style={inputStyle} />
        </label>
        <label style={fieldStyle}>
          <span>Valor</span>
          <input
      ...
      <label style={fieldStyle}>
        ...
        <textarea value={observacoes} onChange={(event) => setObservacoes(event.target.value)} rows={3} style={{ ...inputStyle, resize: 'vertical' }} />
      </label>
